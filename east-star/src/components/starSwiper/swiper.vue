<template>
  <div class="c-swipe" @touchstart="handleTouchstart" @touchmove="handleTouchmove" @touchend="handleTouchend" @touchcancel="handleTouchend">
    ...
    <div v-if="pagination" class="c-swipe-pagination">
      <div class="c-swipe-pagination-bar myP" v-if="type == 1">
        <i v-for="item in length" :class="['c-swipe-pagination-item', item - 1 === insideValue ? 'active': '']" :key="item"></i>
        <!-- <p v-for="(item,index) in length" :class="['c-swipe-pagination-item', item - 1 === insideValue ? 'active': '']" :key="item">{{index+1}}/{{length}}</p> -->
      </div>
      <div class="c-swipe-pagination-bar" v-else>
        <!-- <i v-for="item in length" :class="['c-swipe-pagination-item', item - 1 === insideValue ? 'active': '']" :key="item"></i> -->
        <p v-for="(item,index) in length" :class="['c-swipe-pagination-item', item - 1 === insideValue ? 'active': '']" :key="item">{{index+1}}/{{length}}</p>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    this.init();
    this.initOnce();
    $(".c-swipe-item").height($(window).height())
    $(".c-swipe-item-copy").height($(window).height())
    // $(".c-swipe-item").width($(window).width())
    // $(".c-swipe-item-copy").width($(window).width())
  },
  destroyed() {
